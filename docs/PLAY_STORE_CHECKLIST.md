...
- **Feature graphic**: 1024×500 px — gata generat, `store/feature-graphic.png` (foloseste iconita si paleta reala a aplicatiei). Verifica-l vizual inainte sa-l urci, il poti regenera/ajusta oricand.
- **Screenshot-uri telefon**: gata 5, in `store/screenshots/` — din aplicatia instalata (fara interfata browserului), cu poze fara continut personal. Acopera minimul de 2 cerut de Play Console; poti adauga altele daca vrei sa arati si alte ecrane (Persoane cunoscute, Statistici etc.).
...
- **Permisiuni**: verifica lista REALA in `android/app/src/main/AndroidManifest.xml`, nu din memorie. Azi sunt cinci: `INTERNET` (incarcare resurse aplicatie, nu date personale), `READ_MEDIA_IMAGES`, `READ_MEDIA_VISUAL_USER_SELECTED` (Android 14+, cazul "acces limitat"), `READ_EXTERNAL_STORAGE` (`maxSdkVersion="32"`) si `ACCESS_MEDIA_LOCATION` (coordonatele GPS deja scrise in poze, citite pentru ecranul "Locatii" — Android 10+ le sterge din fisierul servit aplicatiei fara ea). Numele localitatilor NU cer nimic in plus in formularul de siguranta a datelor: se cauta intr-o lista mondiala inclusa in pachet (GeoNames cities500 — toate tarile, localitatile de peste ~500 de locuitori plus resedintele; CC BY 4.0, adusa de CI — vezi pasul "Bundle place names" din workflow-uri si core/placeNames.ts), fara nicio interogare in retea. A existat scurt si o varianta online, prin serviciul de geocodare al sistemului; a fost scoasa fiindca strada dedusa dintr-o coordonata cu eroare de sute de metri e o presupunere, nu un fapt — vezi istoricul git daca vreodata se pune problema reintroducerii ei. Cele trei de acces la poze sunt cerute de Supervizorul galeriei si de "Adu pe perioade" (`MediaLibraryPlugin.kt`), care citesc din MediaStore pe intervale de timp — deci aplicatia CHIAR cere acces la galerie, pe langa selectorul de sistem, iar formularul trebuie sa spuna asta. Pozele raman procesate local; permisiunea e despre citire, nu despre transmitere. `com.android.vending.BILLING` se adauga singura la merge-ul de manifest, din biblioteca Play Billing.
